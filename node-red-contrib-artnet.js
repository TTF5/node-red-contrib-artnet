...
module.exports = function (RED) {
  function ArtnetOutNode(config) {
    ...
    this.sendState = function () {
      ...
      for (var i = 0; i < 512; i++) {
        if (node.nodeData[i] !== null) {
          formattedNodeData.push(node.nodeData[i]);
        } else {
          formattedNodeData.push(undefined);
        }
      }
      node.send({
    ...
    this.on("input", function (msg) {
      ...
      if (payload.start_buckets && Array.isArray(payload.start_buckets)) {
        for (var i = 0; i < payload.start_buckets.length; i++) {
          ...
          node.set(
            payload.start_buckets[i].channel,
            payload.start_buckets[i].value
          );
        }
        node.sendData();
      }
    ...
    this.fadeToValue = function (channel, new_value, transition_time) {
      ...
      var diff = new_value - oldValue;
      if (Math.abs(diff) / steps <= 1) {
        steps = diff;
      }

      var valueStep = diff / steps;
    ...
    this.moveToPointOnArc = function (
      _cv_theta,
      _cv_phi,
      _tilt_nv,
      _pan_nv,
      ...
      // current value
      var cv_theta = artnetutils.channelValueToRad(
        _cv_theta,
        arcConfig.tilt_angle
      ); //tilt
      var cv_phi = artnetutils.channelValueToRad(_cv_phi, arcConfig.pan_angle); // pan

      // target value
      var nv_theta = artnetutils.channelValueToRad(
        _tilt_nv,
        arcConfig.tilt_angle
      );
      ...
      // center value
      var tilt_center = artnetutils.channelValueToRad(
        _tilt_center,
        arcConfig.tilt_angle
      ); //tilt
      var pan_center = artnetutils.channelValueToRad(
        _pan_center,
        arcConfig.pan_angle
      ); // pan
      ...
      artnetutils.log(
        ...
        nv_phi,
        "\n" + "newPoint2: ",
        utils.radiansToDegrees(nv_theta),
        utils.radiansToDegrees(nv_phi),
        "\n" + "centerPoint: ",
        tilt_center,
        pan_center
      );
      ...
      var centerPoint = utils.toCartesian({
        phi: pan_center,
        theta: tilt_center,
      });
      var vn = centerPoint;
      ...
      var currentPoint1 = utils.rotatePoint_xy_quarterion(
        currentPoint,
        backVector
      );
      var newPoint1 = utils.rotatePoint_xy_quarterion(newPoint, backVector);
      var centerPoint1 = utils.rotatePoint_xy_quarterion(
        centerPoint,
        backVector
      );
      ...
      var radius = utils.getDistanceBetweenPointsInCartesian(
        currentPoint,
        centerPoint
      );
      var radius2 = utils.getDistanceBetweenPointsInCartesian(
        newPoint,
        centerPoint
      );
      ...
      artnetutils.log(
        "5 -> parametric equation startT and endT calculation \n"
      );
      //find t parameter for start and end point
      var currentT =
        (Math.acos(currentPoint.x / radius) + 2 * Math.PI) % (2 * Math.PI);
      var newT = (Math.acos(newPoint.x / radius) + 2 * Math.PI) % (2 * Math.PI);
      artnetutils.log("T parameters rad", radius, currentT, newT);
      artnetutils.log(
        "T parameters degree",
        utils.radiansToDegrees(currentT),
        utils.radiansToDegrees(newT)
      );
      artnetutils.log("*************************************");

      var actualAngle = newT - currentT;
      var angleDelta =
        Math.abs(actualAngle) <= Math.PI
          ? actualAngle
          : -(actualAngle - Math.PI);
      ...
      artnetutils.log(
        ...
        angleDelta,
        angleStep,
        utils.radiansToDegrees(angleDelta)
      );
      ...
      for (var i = startStep; i <= endStep; i++) {
        ...
        timeoutID = setTimeout(
          function (t) {
            ...
            var iterationPoint = utils.getIterationPoint(
              t,
              radius,
              backVector,
              movement_point
            );
            var tilt = artnetutils.validateChannelValue(
              artnetutils.radToChannelValue(
                iterationPoint.theta,
                arcConfig.tilt_angle
              )
            );
            var pan = artnetutils.validateChannelValue(
              artnetutils.radToChannelValue(
                iterationPoint.phi,
                arcConfig.pan_angle
              )
            ...
            artnetutils.log(
              "sphericalP     ",
              "r: ",
              parseFloat(iterationPoint.r).toFixed(4),
              "theta:",
              parseFloat(iterationPoint.theta).toFixed(4),
              "phi:",
              parseFloat(iterationPoint.phi).toFixed(4),
              "\n",
              "T:             ",
      ...
      if (endStep == 1) {
        var tilt = artnetutils.validateChannelValue(
          artnetutils.radToChannelValue(nv_theta, arcConfig.tilt_angle)
        );
        var pan = artnetutils.validateChannelValue(
          artnetutils.radToChannelValue(nv_phi, arcConfig.pan_angle)
        );
        ...
        // add close callback to set channels to new_value in case redeploy and all timeouts stopping
        node.closeCallbacksMap[arcConfig.pan_channel] = node.closeCallbacksMap[
          arcConfig.tilt_channel
        ] = function () {
          node.set(arcConfig.tilt_channel, tilt);
          node.set(arcConfig.pan_channel, pan);
        };
